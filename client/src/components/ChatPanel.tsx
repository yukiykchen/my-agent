...
export default function ChatPanel({ session, onBack }: Props) {
  ...
  const sendMessage = async () => {
    ...
    try {
      // 将 caseId 注入消息，使 Agent 知道查询哪个案件的证据
      const enrichedText = `[当前案件ID: ${currentCaseId}]\n\n${text}`
    ...
    >
      ...
      <header className="chat-header">
        <div className="header-left">
          ...
          <span className="badge case-badge" title={currentCaseId} onClick={() => setShowDashboard(true)} style={{ cursor: 'pointer' }}>
            案件: {currentCaseId.slice(-8)}
          </span>
        </div>
        <div className="header-right">
          <button
            className="icon-btn web-btn"
            onClick={() => setShowWebCapture(true)}
            title="网页取证（截图 / 正文抓取 + 即时固化）"
          >
            ...
            <span style={{ marginLeft: 4, fontSize: 12 }}>网页取证</span>
          </button>
          <button
            className="icon-btn evidence-btn"
            onClick={() => setShowEvidenceBrowser(true)}
            title="查看已固化证据列表"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <span style={{ marginLeft: 4, fontSize: 12 }}>证据列表</span>
          </button>
          <button
          ...
          >
            ...
            <span style={{ marginLeft: 4, fontSize: 12 }}>报告</span>
          </button>
          <button
            className="icon-btn live-btn"
            onClick={() => setShowLiveCapture(true)}
            title="直播取证（5 秒分段即时固化）"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="10" />
              <circle cx="12" cy="12" r="4" fill="currentColor" />
            </svg>
            <span style={{ marginLeft: 4, fontSize: 12 }}>直播取证</span>
          </button>
      ...
      {showEvidenceBrowser && (
        <div className="live-capture-overlay" onClick={(e) => {
          if (e.target === e.currentTarget) setShowEvidenceBrowser(false)
        }}>
          <div className="live-capture-modal">
            <EvidenceBrowser caseId={currentCaseId} onClose={() => setShowEvidenceBrowser(false)} />
          </div>
        </div>
      )}
      ...
      {showDashboard && (
        <div className="live-capture-overlay" onClick={(e) => {
          ...
          <div className="live-capture-modal">
            <CaseDashboard caseId={currentCaseId} onClose={() => setShowDashboard(false)} />
          </div>
        </div>
      )}
      ...
      {showWebCapture && (
        <div className="live-capture-overlay" onClick={(e) => {
          if (e.target === e.currentTarget) setShowWebCapture(false)
        }}>
          <div className="live-capture-modal">
            <WebCapturePanel caseId={currentCaseId} onClose={() => setShowWebCapture(false)} />
          </div>
        </div>
      )}
      ...
      {showReportViewer && (
        <div className="live-capture-overlay" onClick={(e) => {
          if (e.target === e.currentTarget) setShowReportViewer(false)
        }}>
          <div className="live-capture-modal">
            <ReportViewer caseId={currentCaseId} onClose={() => setShowReportViewer(false)} />
          </div>
        </div>
      )}
      ...
      {showLiveCapture && (
        <div className="live-capture-overlay" onClick={(e) => {
          if (e.target === e.currentTarget) setShowLiveCapture(false)
        }}>
          <div className="live-capture-modal">
            <LiveCapturePanel caseId={currentCaseId} onClose={() => setShowLiveCapture(false)} />
          </div>
        </div>
      )}
